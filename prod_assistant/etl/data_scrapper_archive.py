...
import undetected_chromedriver as uc # not to get detected by the browser
...
class FlipkartScraper:
    ...
    def scrape_flipkart_products(self, query, max_products=1, review_count=2):
      # retrieve 1 product with 2 reviews
        """Scrape Flipkart products based on a search query.
        """
        options = uc.ChromeOptions()
        driver = uc.Chrome(options=options,use_subprocess=True)
        search_url = f"https://www.flipkart.com/search?q={query.replace(' ', '+')}" # get request
        driver.get(search_url)
        time.sleep(4) # wait for the page to load

        try:
            driver.find_element(By.XPATH, "//button[contains(text(), '✕')]").click() # close the popup
        except Exception as e:
            print(f"Error occurred while closing popup: {e}")
